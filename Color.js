typeof module < "u" ? module.exports = Color : this.Color = Color;
function Color() {
    var d, a, o;
    if (arguments.length === 1) {
        var e = arguments[0].toLowerCase();
        e.match(/^#[0-9a-f]{6}$/i) && (e = /^#?([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i.exec(e),
        e && e.length === 4 && (d = parseInt(e[1], 16),
        a = parseInt(e[2], 16),
        o = parseInt(e[3], 16)))
    } else
        arguments.length === 3 && (d = arguments[0],
        a = arguments[1],
        o = arguments[2]);
    this.r = ~~d || 0,
    this.g = ~~a || 0,
...
Color.prototype.distance = function(d) {
    var a = 0;
    return a += Math.pow(this.r - d.r, 2),
    a += Math.pow(this.g - d.g, 2),
    a += Math.pow(this.b - d.b, 2),
    Math.abs(Math.sqrt(a))
}
,
Color.prototype.add = function(d, a, o) {
    this.r += d,
    this.g += a,
    this.b += o,
    this.r < 0 ? this.r = 0 : this.r > 255 && (this.r = 255),
    this.g < 0 ? this.g = 0 : this.g > 255 && (this.g = 255),
    this.b < 0 ? this.b = 0 : this.b > 255 && (this.b = 255)
}
,
Color.prototype.toHexa = function() {
    var d = (~~this.r || 0).toString(16)
      , a = (~~this.g || 0).toString(16)
      , o = (~~this.b || 0).toString(16);
...
Color.prototype.getName = function() {
    var d = this.toHexa(), a = 256, o;
    for (var e in Color.map)
        if (Color.map.hasOwnProperty(e)) {
            var p = Color.map[e];
            if (p.r === this.r && p.g === this.g && p.b === this.b)
                return e;
            var M = this.distance(p);
            M < a && (a = M,
            o = e)
        }
    return o ? o = "A shade of " + o : o = this.toHexa(),
    o
}
,
Color.map = {},
Color.addToMap = function(d, a) {
    Color.map[a] = new Color(d)
}
,
